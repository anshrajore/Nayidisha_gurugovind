...
import { appendJobToGoogleSheets } from "@/utils/googleSheets";
...
const PostJob = () => {
  ...
  const handleMultiSelectChange = (name: keyof FormDataState, value: string, checked: boolean) => {
    if (checked) {
      setFormData(prev => ({
        ...prev,
      ...
      setFormData(prev => ({
        ...prev,
        [name]: (prev[name] as string[]).filter(item => item !== value)
      }));
    }
  };
  ...
  const handleFileUpload = (name: string, e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (name === 'siteImages') {
        setFormData(prev => ({
          ...prev,
          siteImages: [...prev.siteImages, fileURL]
        }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Prefer posting to Sheets even if DB insert fails.
      const { data: sessionData } = await supabase.auth.getSession();
      const session = sessionData.session;

      const employerId = session?.user?.id ?? null;
      ...
      try {
        await appendJobToGoogleSheets({
          ...sheetPayload,
          created_at: new Date().toISOString(),
        });
      } catch (sheetError) {
        console.warn("Google Sheets append failed:", sheetError);
      }

      // Only insert into Supabase when authenticated, using valid DB columns only.
      if (!employerId) {
        toast({
          title: "Saved to Google Sheet",
          description: "Sign in as an employer to also publish it in the app.",
          variant: "default",
        });
        navigate(`/jobs`);
        return;
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-col items-start gap-4">
                      ...
                      <input
                        ...
                        className="hidden"
                        aria-label="Upload Workplace Photos"
                      />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="hidden"
                      aria-label="Upload Workplace Video"
                    />
